...
if __name__ == '__main__':
    ...
    gps_x.append(0)
    gps_y.append(0)
    gps_heading.append(0)
    td = 0

    gps_path_data['dx_dash'] = gps_path_data['utm_data_x'].diff()
    gps_path_data['dy_dash'] = gps_path_data['utm_data_y'].diff()

    # dx_dash = gps_path_data['dx_dash']
    # dy_dash = gps_path_data['dy_dash']
    gps_path_data[0, 'dx_dash'] = 0
    gps_path_data[0, 'dy_dash'] = 0


    gps_path_data['gps_x'] = gps_path_data['dx_dash'].cumsum()
    gps_path_data['gps_y'] = gps_path_data['dy_dash'].cumsum()

    # for i in range(1, len(gps_path_data)):
    #
    #     # heading = np.arctan2(gps_path_data['dy_dash'], gps_path_data['dx_dash'])
    #
    #     dx = gps_x[i - 1] + dx_dash[i]
    #     dy = gps_y[i - 1] + dy_dash[i]
    #     # gps_heading.append(heading)
    #     gps_x.append(dx)
    #     gps_y.append(dy)

        # dd = np.sqrt(gps_path_data['dx_dash'] ** 2 + gps_path_data['dy_dash'] ** 2)
        # td = td + dd

    # print(td)
    plotPaths(gps_x, gps_y, 2)
    plt.show()

    gps_df_save = pd.DataFrame(gps_path_data[['gps_x', 'gps_y']])
    print(gps_df_save)

    # return gps_x, gps_y, gps_heading
