...
if __name__ == '__main__':
    ...
    name_list = ['xyX', 'xyY', 'xzX', 'xzZ', 'yzY', 'yzZ']
    for index_1 in range(len(name_list)):
        acc_file_name = "./Calibration/Stationary/accel_" + name_list[index_1] + ".csv"
        acc = getData(acc_file_name)
        gyr_file_name = "./Calibration/Stationary/gyro_" + name_list[index_1] + ".csv"
        ...
        acc.loc[0, 'delta_time'] = 0
        acc['time_series'] = acc['delta_time'].cumsum()

        gyr.loc[0, 'delta_time'] = 0
        gyr['time_series'] = gyr['delta_time'].cumsum()

        plt.figure((index_1 * 2) + 1)
        plt.plot(acc['time_series'], acc['field.linear_acceleration.x'], label="Ax")
        plt.plot(acc['time_series'], acc['field.linear_acceleration.y'], label="Ay")
        plt.plot(acc['time_series'], acc['field.linear_acceleration.z'], label="Az")
        plt.xlabel('Time')
        plt.ylabel('Linear Acceleration')

        plt.figure((index_1 * 2) + 2)
        plt.plot(gyr['time_series'], gyr['field.angular_velocity.x'], label="Wx")
        plt.plot(gyr['time_series'], gyr['field.angular_velocity.y'], label="Wy")
        plt.plot(gyr['time_series'], gyr['field.angular_velocity.z'], label="Wz")
